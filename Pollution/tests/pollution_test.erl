...
get_air_quality_index_test() ->
  ...
  M1 = pollution:add_station("Stacja 1", {1, 1}, M0),

  Time1 = {{2026, 4, 8}, {12, 0, 0}},
  Time2 = {{2026, 4, 8}, {13, 0, 0}},


  M2 = pollution:add_value("Stacja 1", Time1, "PM10", 75.0, M1),
  M3 = pollution:add_value("Stacja 1", Time1, "PM2.5", 60.0, M2),
  M4 = pollution:add_value("Stacja 1", Time1, "temperatura", 20.0, M3),
  M5 = pollution:add_value("Stacja 1", Time2, "PM2.5", 90.0, M4),

  ?assertEqual(200.0, pollution:get_air_quality_index("Stacja 1", Time1, M5)),
  ?assertEqual(200.0, pollution:get_air_quality_index({1, 1}, Time1, M5)),

  Time3 = {{2026, 4, 8}, {23, 0, 0}},

  ?assertEqual({error, no_measurements}, pollution:get_air_quality_index("Stacja 1", Time3, M5)),
  ?assertEqual({error, station_not_found}, pollution:get_air_quality_index("Random", Time1, M5)).
